...
func (m *ArticleMutation) SetContent(sc *schema.ArticleContent) {
	m.content = &sc
}

// Content returns the value of the "content" field in the mutation.
func (m *ArticleMutation) Content() (r *schema.ArticleContent, exists bool) {
	v := m.content
	if v == nil {
		return
	}
...
// If the Article object wasn't provided to the builder, the object is fetched from the database.
// An error is returned if the mutation operation is not UpdateOne, or the database query fails.
func (m *ArticleMutation) OldContent(ctx context.Context) (v *schema.ArticleContent, err error) {
	if !m.op.Is(OpUpdateOne) {
...
func (m *ArticleMutation) SetField(name string, value ent.Value) error {
	...
	case article.FieldDescription:
		...
		return nil
	case article.FieldContent:
		v, ok := value.(*schema.ArticleContent)
		if !ok {
			return fmt.Errorf("unexpected type %T for field %s", value, name)
		}
		m.SetContent(v)
